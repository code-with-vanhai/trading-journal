                        ...
                        {statsData.map((stat, index) => {
                            ...
                            return (
                                <div key={index} className="p-4 flex flex-col items-center">
                                    <div className="w-16 h-16 rounded-full bg-blue-100 dark:bg-blue-500/10 flex items-center justify-center mb-4 neon-glow">
                                        <IconComponent className={`w-8 h-8 text-blue-600 dark:text-blue-400`} />
                                    </div>
                                    <p className={`text-4xl font-bold mb-2 ${stat.color === 'green' ? 'text-green-600 dark:text-green-400' : 'text-blue-600 dark:text-blue-400'} flex items-center justify-center drop-shadow-lg`}>
                                        {stat.value}
                                        {stat.hasArrow && <IconArrowUp className="ml-2 text-green-600 dark:text-green-400 w-6 h-6" />}
                                    </p>
